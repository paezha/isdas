---
title: "31 Chapter: Spatially Continuous Data I"
author: 
- name: Antonio Páez
...
![Figure 1. Voronoi polygons by radial growth](31-Figure-1.jpg)
...
$\hat{z}_p = z_{p_g}$ for $p_g$ with $d_{pp_g}<d_{pp_k}\forall{k}$.
...
![Figure 2. A field according to Voronoi polygons](31-Figure-2.jpg)
...
Notice the dots where the observations are - the value of the field is known there. We can explore the effect of changing the parameter for the power, by using $\gamma = 0.5, 1, 2,$ and $5$:
```{r}
...
$w_{pi} = 1$ if $i$ is one of $k$-th nearest neighbors of $p$ for a given $k$, and $w_{pi} = 0$ otherwise.
