...
$$
\textrm{Find the values of }\beta\textrm{ that minimize }\sum_{i=1}^n{\epsilon_i^2}
$$
...
$$
Var[\epsilon_i|X] = \sigma^2 \textrm{ }\forall i
$$

5) The residuals are independent, that is, they are not correlated among them:
$$
E[\epsilon_i\epsilon_j|X] = 0 \textrm{ }\forall i\neq j
$$
...
im3 <- localmoran.map(Hamilton_CT,
               Hamilton_CT.w,
               "POP_DENSITY",
               by = "TRACT")
...
(\beta_0 + \beta_2) + (\beta_1 + \beta_2)x_i + \epsilon_i \textrm{ if census tract } i \textrm{ is in the CBD}\\
(\beta_0 + \beta_3) + (\beta_1 + \beta_5)x_i + \epsilon_i \textrm{ if census tract } i \textrm{ is in the Suburbs}\\
\beta_0  + \beta_1x_i + \epsilon_i \textrm{ otherwise}\\
\end{array}
$$
