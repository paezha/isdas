...
```{r}
# We can use the arguments xlab and ylab in `moran.plot()` 
# to change the labels for the two axes of the plot
mp <- moran.plot(Hamilton_CT$POP_DENSITY, 
                 Hamilton_CT.w, 
...
```{r}
# Use the pipe operator to pass the dataframe as an argument to `mutate()`,
# which is used to create new variables.
Hamilton_CT <- Hamilton_CT %>% 
  # Create a mean-centered variable that is standardized by the variance.
  mutate(Z = (POP_DENSITY - mean(POP_DENSITY)) / var(POP_DENSITY),
         # Calculate the spatial moving average of variable `Z`.
         SMA = lag.listw(Hamilton_CT.w, Z), 
         # The function `case_when()` is used to evaluate several logical
         # conditions and respond to them. 
...
bscols(
  ...
  # Create a `plotly` object using the dataframe as an input.
  # The pipe operator passes this object to the function `add_markers()`;
  # this function is similar to the `geom_point()` function in `ggplot2`
  # and it draws objects on the blank plot created by `plot_ly()`
  plot_ly(df_msc.sd) %>% 
    add_markers(trace = "scatter",
                x = ~Z, 
                y = ~SMA, 
                color = ~POP_DENSITY, 
                size = ~(Z * SMA), 
    ...
    hide_colorbar() %>%     
    # Highlight observations when selected.
    highlight("plotly_selected"),
  # The second plot is a choropleth map
  # Create a `plotly` object using the dataframe as an input. 
  # The pipe operator passes this object to the function `add_sf()`; 
  # this function is similar to the `geom_sf()` functions in `ggplot2`
  # and it draws a simple features object on the blank plot created
  # by `plot_ly()`
  plot_ly(df_msc.sd) %>% 
    add_sf(split = ~TRACT,
           color = ~POP_DENSITY, 
...
```{r}
# The function `add_sf()` draws a simple features object, 
# similar to `geom_sf()` in `ggplot2`. We "split" observations 
# based on their p-values: if the p-value is less than 0.05, 
# the condition is "TRUE" and otherwise it is "FALSE". Finally, 
#we color the zones based on their `Type`: that is, whether they 
# are High & High according to the local statistic, or Low & Low, etc.
im1 <- plot_ly(Hamilton_CT) %>%
    add_sf(type = "scatter",
           split = ~(p.val < 0.05), 
           color = ~Type, 
           colors = c("red", 
...
htmlwidgets::saveWidget(widget = im1, 
                        file = "im1.html")
webshot2::webshot(url = "im1.html", 
                  file = "im1.png", 
                  delay = 1, 
                  zoom = 4, 
                  vheight = 500)
...
```{r}
# A function is a way of packaging a set of standard instructions.
# Here, we package all the steps we used above to create the map 
# of the local Moran coefficients in a new function called 
# `localmoran.map()`
...
im2 <- localmoran.map(Hamilton_CT, Hamilton_CT.w, 
               "POP_DENSITY", 
               by = "TRACT")

# This code takes a screen capture of the figure to include it in the 
# pdf file when knitting
htmlwidgets::saveWidget(widget = im2, 
                        file = "im2.html")
webshot2::webshot(url = "im2.html", 
                  file = "im2.png", 
                  delay = 1, 
                  zoom = 4, 
                  vheight = 500)
...
```{r}
# Define how variables in the table are represented in the plot:
# for instance, the variable `x` corresponds to the x axis. Next,
# define the properties of the markers, or geometric objects in 
# the plot. For example, their color will be proportional to variable `z` 
im3 <- plot_ly(df1_simulated, 
        x = ~x, 
        y = ~y, 
        z = ~z, 
        marker = list(color = ~z, 
...
# This code takes a screen capture of the figure to include it in the 
# pdf file when knitting
htmlwidgets::saveWidget(widget = im3, 
                        file = "im3.html")
webshot2::webshot(url = "im3.html", 
                  file = "im3.png", 
                  delay = 1, 
                  zoom = 4, 
                  vheight = 500)
...
# Select a radius
r <- 10
# Extract observations that are within a radius of `r` 
# from focal point `xy0` (note that sqrt((x - xy0)^2 + (x - xy0)^2)
# is Pythagoras's formula for calculating the distance between 
# two points; if this distance is less than `r`, the point is kept)
df1_simulated %>% 
...
# This code takes a screen capture of the figure to include it in the 
# pdf file when knitting
htmlwidgets::saveWidget(widget = im4, 
                        file = "im4.html")
webshot2::webshot(url = "im4.html", 
                  file = "im4.png", 
                  delay = 1, 
                  zoom = 4, 
                  vheight = 500)
...
# Find all nearest neighbors that are within 0 and 10 units 
# of distance away from every observation.
...
```{r}
# The arguments of this function are a spatial variable 
# and a list of spatial weights
...
# This code takes a screen capture of the figure to include it in the 
# pdf file when knitting
htmlwidgets::saveWidget(widget = im5, 
                        file = "im5.html")
webshot2::webshot(url = "im5.html", 
                  file = "im5.png", 
                  delay = 1, 
                  zoom = 4, 
                  vheight = 500)
...
# This code takes a screen capture of the figure to include it in the 
# pdf file when knitting
htmlwidgets::saveWidget(widget = im6, 
                        file = "im6.html")
webshot2::webshot(url = "im6.html", 
                  file = "im6.png", 
                  delay = 1, 
                  zoom = 4, 
                  vheight = 500)
