...
$w_{ij} = 1$ if $A_i$ and $A_j$ share an edge, and $w_{ij} = 0$ otherwise.
...
$w_{ij} = 1$ if $A_i$ and $A_j$ share an edge or a vertex, and $w_{ij} = 0$ otherwise.
...
The total number of neighbors of $A_i$ is given by: $\sum_{j=1}^{n}{w_{ij}}$.
...
map <- ggplotly(map, tooltip = c("TRACT", "POP_DENSIT"))
...
map.sma <- ggplotly(map.sma, tooltip = c("TRACT", "POP_DENSIT.sma"))

# This code takes a screen capture of the figure to include it in the 
# pdf file when knitting
htmlwidgets::saveWidget(widget = map.sma, 
                        file = "map.sma.html")
webshot2::webshot(url = "map.sma.html", 
                  file = "map.sma.png", 
                  delay = 1, 
                  zoom = 4, 
                  vheight = 500)
...
$w_{ij} = 1$ if inter-centroid distance $d_{ij} \leq \delta$ and $w_{ij} = 0$ otherwise, where $\delta$ is a distance threshold.
