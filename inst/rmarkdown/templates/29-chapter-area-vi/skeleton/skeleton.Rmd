...
# Create a `plotly` object with the dataframe and plot the 
# simple features object with colors per the sign of the residuals
# (negative residuals = FALSE, positive residuals = TRUE)
im1 <-  plot_ly(HamiltonDAs) %>%
...
p + geom_abline(slope = mod.of1$coefficients[2], 
                intercept = mod.of1$coefficients[1], 
                color = "blue", size = 1) +
...
p + 
  geom_line(data = data.frame(x = seq(0, 11, 0.1), 
                              y = poly.fun), 
            aes(x = x, y = y),
            color = "blue", 
            size = 1) + 
...
```{r}
moran.test(model.sem2$residuals,
           HamiltonDAs.w, 
           alternative = "less")
```
